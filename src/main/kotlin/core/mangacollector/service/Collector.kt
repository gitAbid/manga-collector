...
class Collector(val luRepo: LatestUpdateRepository,
    ...
    fun brokenUrlFixer() {
        ...
        for (x in 1..updates.totalPages) {
            ...
            updates.content.forEach { update ->
                val mangas = mangaRepository.findByMangaUrl(update.mangaUrl)
    ...
    fun brokenUrlFixerFull() {
        ...
        for (x in urlFixerStartPage..updates.totalPages) {
            ...
            updates.content.forEach { update ->
                val mangas = mangaRepository.findByMangaUrl(update.mangaUrl)
                if (mangas != null && mangas.isNotEmpty()) mangas.first().let { manga ->
                    logger.info("Fixing broken chapter for manga : ${manga.mangaName}")
                    val updatedChapters = LinkedHashSet<Chapter>()
                    val chapters = manga.chapters
                    for (chapter in chapters) {
        ...
        timeElapsedToExecute {
            for (currentPage in firstPage..lastPage) {
                ...
                elements.forEach {
                    ...
                    latestChapterUrl = latestChapterUrl.replace(Regex("(s\\d+\\.m)"), "s8.m").replace(Regex("(\\d+\\.com)"), "8.com")
                    latestChapterUrl = latestChapterUrl.replace("https://bu.mkklcdnbuv8.com", "https://bu.mkklcdnbuv1.com")
    ...
    fun timeElapsedToExecute(function: () -> Unit) {
        ...
        val diffInMillisec: Long = end.time - start.time
    ...
    fun collectTrending() {
        ...
        trendingDoc?.forEach { trend ->
            ...
            mangaRepository.findByMangaUrl(url)?.first()?.let {
                it.trending = true
                mangaRepository.save(it)
            }
        }
